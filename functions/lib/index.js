...
exports.analyzeLink = (0, https_1.onCall)({ secrets: [geminiApiKey] }, async (request) => {
    ...
    if (!url || typeof url !== 'string') {
        throw new https_1.HttpsError('invalid-argument', 'The function must be called with a string "url" argument.');
    }
    const keyVal = geminiApiKey.value();
    if (!keyVal) {
        throw new https_1.HttpsError('failed-precondition', 'GEMINI_API_KEY secret is not configured in Firebase.');
    }
...
URL: ${url}`,
        ...
        throw new https_1.HttpsError('internal', error.message || 'Failed to analyze link using Gemini.');
    }
});
